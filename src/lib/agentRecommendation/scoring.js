...
import { TASK_KEYWORDS, OUTPUT_KEYWORDS } from './rules.js'
...
export function scoreAgent(agent = {}, preferences = {}, weights = DEFAULT_RECOMMENDATION_WEIGHTS) {
  ...
  const taskTypes = [...new Set([...(goal?.taskTypes || []), ...extras.flatMap((extra) => extra.taskTypes || [])])]
  const outputFormats = [...new Set(extras.map((extra) => extra.outputFormat).filter((format) => format && format !== 'any'))]
  const matchedSignals = { taskTypes: [], freeTextTerms: [] }
  let score = 0
  ...
  outputFormats.forEach((format) => {
    const keywords = OUTPUT_KEYWORDS[format] || [format]
    if (keywords.some((keyword) => text.includes(normalize(keyword)))) {
      score += weights.outputFormat
      matchedSignals.outputFormat = format
    }
  })
  ...
  if (preferences.experienceLevel && preferences.experienceLevel !== 'any') {
    ...
    if (terms.some((term) => text.includes(term))) score += weights.experience
  }

  if (preferences.budgetPreference === 'quick' && ['summary', 'reply', 'regex', 'email', 'tone'].some((term) => text.includes(term))) score += weights.urgency
  if (preferences.budgetPreference === 'thorough' && ['audit', 'report', 'plan', 'architecture', 'analysis'].some((term) => text.includes(term))) score += weights.urgency
...
export function getMaxPossibleScore(preferences = {}, weights = DEFAULT_RECOMMENDATION_WEIGHTS) {
  ...
  const taskCount = (getGoal(preferences.primaryGoal)?.taskTypes?.length || 0) + extras * 2
  return weights.exactCategory + weights.goalCategory + Math.max(1, taskCount) * weights.taskType + Math.max(1, extras) * weights.outputFormat + weights.providerExact + weights.freeTextName * 3 + weights.experience + weights.urgency
}
...
export function recommendAgents(agents = [], preferences = {}, options = {}) {
  ...
  return agents
    ...
    .filter((result) => result.score >= (options.minScore ?? MIN_CONFIDENT_SCORE))
    .map((result) => ({ ...result, matchPercentage: normalizeScore(result.score, maxScore) }))
